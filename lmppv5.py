...
# Inicializar a lista de quantidades de água nos tubos
quantidades_de_agua = []

# Inicializar a lista de coeficientes de variação
...
for nome_arquivo in os.listdir(caminho_diretorio):
    ...
    if nome_arquivo.endswith('.jpg') or nome_arquivo.endswith('.png'):
        ...
        # Salvar o coeficiente de variação em um arquivo CSV
        df = pd.DataFrame({'coeficiente_variacao': coeficientes_variacao})
        nome_arquivo_sem_extensao = os.path.splitext(nome_arquivo)[0]
        df.to_csv(f'{nome_arquivo_sem_extensao}_cv.csv', index=False)
